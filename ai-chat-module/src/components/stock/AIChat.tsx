...
interface AIStockFilters {
  minVolumeRatio: number
  maxVolumeRatio: number
  minMainInflow: number
  maxMainOutflow: number
  minMainNetInflow: number
  minPriceChange: number
  maxPriceChange: number
  minVolatility: number
  maxVolatility: number
  openCloseRelation: 'above' | 'below' | 'any'
  priceVsMainCost: 'above' | 'below' | 'near' | 'any'
  minRSI: number
  maxRSI: number
  macdSignal: 'golden' | 'death' | 'any'
  volumeWeight: number
  mainForceWeight: number
  priceWeight: number
...
interface AIChatProps {
  onAnalyze: (filters: AIStockFilters) => void
  onReset: () => void
  isLoading?: boolean
}
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      role: 'assistant',
      content: '您好！我是您的AI投资顾问，可以帮助您分析股票和提供投资建议。您可以直接输入问题，比如"推荐一些科技股"或"分析一下000001"。',
      timestamp: new Date()
    }
  ])
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Image 
            ...
            className="rounded"
          />
          <h3 className="text-xl font-semibold">AI投资顾问</h3>
        </div>
