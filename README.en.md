...
3.  Configure the database (Prisma):
    ```bash
    # The project uses a SQLite database, so no external connection configuration is needed.
...
- **Next.js**: Modern React framework with App Router support.
...
- **Python + pyshark**: For backend network packet capture and analysis.

### AI Integration
- **AI SDKs**: Integrates AI chat and analysis capabilities.
...
### Data Storage
- **SQLite**: For the database, managed by Prisma.
